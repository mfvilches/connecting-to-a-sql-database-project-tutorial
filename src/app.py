...
def connect():
    global engine
    try:
        conn = f'postgresql://{os.getenv('DB_USER')}:{os.getenv('DB_PASSWORD')}@{os.getenv('DB_HOST')}/{os.getenv('DB_NAME')}'
        print('conexión terminada')
        engine = create_engine(conn, isolation_level='AUTOCOMMIT')
        engine.connect()
        print('Conectado!!!')
        return engine
    except Exception as e:
        print(f'Error conectandose a la BBDD')
        print(e)
        return None
# 2) Create the tables
engine = connect()
...
# 3) Insert data
with engine.connect() as conexion:
    conexion.execute(text("""-- publishers
